...
class Game(models.Model):
    ...
    def board(self):
        ...
        so you can ask for the state of a squareat position [y][x]."""
    ...
    def update_after_move(self, move):
        # Update the status if the game, given the last move.
        self.status = self._get_game_status_after_move(move)

    def _get_game_status_after_move(self, move):
        x, y = move.x, move.y
        board = self.board()
        if (board[y][0] == board[y][1] == board[y][2]) or (board[0][x] == board[1][x] == board[2][x]) or (
